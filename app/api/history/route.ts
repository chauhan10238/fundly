...
type FmpRow = { date?: string; datetime?: string; close?: number; price?: number; volume?: number }
...
function subtractRange(range: RangeKey) {
  const end = new Date()
...
function sampleFiveYearRows(rows: FmpRow[]) {
  const byWeek = new Map<string, FmpRow>()
  for (const row of rows) {
    const raw = row.date ?? row.datetime
    if (!raw) continue
    ...
    byWeek.set(`${year}-${Math.floor(dayOfYear / 7)}`, row)
  }
...
function normaliseRows(rows: FmpRow[], range: RangeKey) {
  const ordered = [...rows].sort((a, b) =>
    new Date(a.datetime ?? a.date ?? 0).getTime() -
    new Date(b.datetime ?? b.date ?? 0).getTime()
  )
  const sampled = range === "5Y" ? sampleFiveYearRows(ordered) : ordered
  ...
  }).filter((point): point is { timestamp: string; close: number; volume: number } => point !== null)
}
...
export async function GET(request: NextRequest) {
  ...
  if (!VALID_RANGES.has(range)) {
    return NextResponse.json({ error: "Range must be 1D, 1W, 1M, 1Y or 5Y." }, { status: 400 })
  }
  ...
  const { start, end } = subtractRange(range)
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(!Array.isArray(payload) && payload.error
        ? payload.error
        : `Historical price request failed (${response.status})`)
    }
    if (!Array.isArray(payload)) throw new Error(payload.error || "Historical price data was unavailable.")
    ...
    return NextResponse.json({
      error: error instanceof Error ? error.message : "Unable to retrieve historical prices."
    }, { status: 502 })
  }
}
